...
// We serialize all PATH-mutating tests via PATH_LOCK so no two run concurrently.
#![allow(unsafe_code)]
...
/// Global mutex that serializes all tests that mutate PATH.
...
fn emit_resolve_shells_docket_resolve() {
    ...
    // Prepend tmpdir to PATH so our fake docket is found first.
    ...
    // SAFETY: PATH_LOCK held; no other thread modifies PATH concurrently.
    ...
    // Restore PATH before releasing lock.
