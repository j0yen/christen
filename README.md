...
## christen route — systemd drop-in generator

`christen route` discovers the three canonical headless launch sites
(`claude-build.service`, `claude-dream.service`, `claude-self-review.service`)
and generates systemd drop-in override files that rewrite each `ExecStart=`
to route through `agentns-claude`. It also advises an alias for the
interactive shell site.

### Drop-in format

Each generated drop-in uses the **clear-then-set** pattern required by
systemd to override a multi-valued directive:
...
The bare `ExecStart=` clears the original value; the second line sets the
wrapped command. **Both lines are required and must appear in this order** —
a `systemd-analyze --user verify` gate asserts this.

Drop-ins are written to `~/.config/systemd/user/<unit>.d/10-christen.conf`.
The `.d/` directory is created if absent; re-running `--apply` is idempotent
(identical bytes).

### Print-vs-apply boundary

`christen route` (no flags) **prints only** — it shows the `RoutePlan` and
every drop-in's path + full contents it *would* write, plus the `Advise`
alias snippet for the interactive shell. **Nothing is written to disk.**

`christen route --apply` writes the drop-in files and then **prints** (never
runs) the commands needed to activate them:

```sh
systemctl --user daemon-reload
systemctl --user restart claude-build
# ... (per wired unit)
```

`christen route` **never** runs `daemon-reload`, `enable`, or `restart`.
These lines are printed for the user to review and run manually.

### `systemd-analyze verify` gate

Every generated drop-in is validated by
`systemd-analyze --user verify <unit>` in the integration test suite
(`tests/route_ac5.rs`). The test is skipped (with a logged note) if
`systemd-analyze` is absent on the build box, but **never silently passed**.

### `christen cap` prerequisite

The drop-ins route sessions through `agentns-claude --intent <tag> --`.
For `agentns-claude` to actually create an agent namespace it needs
`cap_sys_admin+ep` (file capability). Without it, `unshare(CLONE_NEWAGENT)`
fails with `EPERM` and the launcher silently falls back to an unwrapped
exec — session ids remain all-zeros.

`christen route --apply` prints a reminder to run `christen cap` after
writing the drop-ins:

```
# NOTE: agentns-claude needs cap_sys_admin+ep to create the agent namespace.
#       Run `christen cap` to check and get the exact setcap command.
```

### Usage

```sh
# Print the route plan + drop-in contents (nothing written to disk)
christen route

# Restrict to one unit
christen route --unit claude-build.service

# Write drop-ins; print (never run) daemon-reload + restart lines
christen route --apply

# Write drop-ins for one unit only (idempotent)
christen route --apply --unit claude-build.service
```

### Shell site (interactive)

The interactive shell site is always discovered and always produces an
`Advise` action whose snippet is a copy-pasteable alias:

```sh
alias claude='agentns-claude --intent interactive --budget wall=7200s,fork=2000 -- claude'
```

This is printed but never written — editing your shell RC is the user's
choice.
